...
<style>
        ...
        .Reward_ul{margin-left:2%;}
        .Reward_ul li{width:48%;margin:10px 1%;}
	</style>
</head>
<body style="background:#eeeeee;">
<?php if(empty($list)): ?><div class="promptbox">
	<img src="../Style/static/images/cw_03.png" class="prompt">
</div>
<h1>奖励已被领取光了！</h1><?php endif; ?>
        	<ul class="Reward_ul">
        	 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><li>
                	<a  href="<?php echo U('Jifen/jifen',array('id'=>$val['id']));?>" class="reward_a">
                    	<img src="<?php echo get_img($val[img],'score_item');?>" height="180"/>
                        <p class="font_p1"><?php echo (strip_tags(mb_substr($val["title"],0,8,'utf-8'))); ?></p>
                        <p class="color_p fl"><?php echo ($val["score"]); ?><span>积分</span></p>
                        <input type="button" value="立即兑换" style="text-align: center"/>
                    </a>
