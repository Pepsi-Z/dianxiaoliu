            ...
            <tr>
                ...
                <div class="explain_col">
                   ...
                   <!-- &nbsp;&nbsp;状态 :
                    ...
                    <option value="0" <?php if($search["status"] == '2'): ?>selected="selected"<?php endif; ?>>处理中</option>
                    <option value="1" <?php if($search["status"] == '3'): ?>selected="selected"<?php endif; ?>>已完成</option>
                    </select>-->
    ...
    <table width="100%" cellspacing="0">
        <thead>
            <tr>
                ...
                <th width="60"><span data-tdtype="order_by" data-field="item_num">数量</span></th>
                <th width="60"><span data-tdtype="order_by" data-field="order_score">积分</span></th>
        ...
        <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr <?php if($val["warning"] == 1 and $val["status"] == 1): ?>class="bg"<?php endif; ?>>
                ...
                <td align="center"><?php if($val['status'] == '1'): ?><span class="red">待发货</span>
                <?php elseif($val['status'] == '2'): ?>待收货<?php else: ?><span class="green">已完成</span><?php endif; ?></td>
                <td align="center"><?php echo ($val["remark"]); ?></td>
                <td align="center"><a href="<?php echo u('score_order/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>">处理</a> | <a href="javascript:void(0);" class="J_confirmurl" data-uri="<?php echo u('score_order/delete', array('id'=>$val['id']));?>" data-acttype="ajax" data-msg="<?php echo sprintf(L('confirm_delete_one'),$val['order_sn']);?>"><?php echo L('delete');?></a></td>
...
<script type="text/javascript"> 
	...
	function showalert()
	{ 
		var url  = "<?php echo U('score_order/order_warning');?>";
    	$.post(url,function(data){
    		if(data.state==1) {
    			popWin("detail");
    			//play_click();
				$('#count_html').html('你有'+data.count+'个积分订单，是否马上处理？');
    		}

    	},'json');
	} 
	function push_qd(){
		window.location.href = "<?php echo U('score_order/index');?>";
	}
	function push_qx(){
