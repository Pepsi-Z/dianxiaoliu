        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbdetail">
                    <tbody>
                        <tr>
                         ...
                         <?php case "2": ?>待配送<?php break;?>
                         <?php case "3": ?>已配送<?php break;?>
            ...
            <!-- 
               ...
               <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbdetail">
                 ...
                 -->
                <?php if(!empty($info["freecode"])): ?><div align="center" style="background:none repeat scroll 0 0 #E6F1F6;font-size:14px;font-weight:bold;padding:10px 0;">配送信息</div>    
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tbdetail">
                    <tbody>
                       <!--  <tr>
                         ...
                         <?php case "3": ?>ems<?php break;?>  
                          ...
                          <tr>
                            ...
                            <td width="22%"> 
                            <?php if($info["userfree"] == '0'): ?>无需配送<?php elseif($info["userfree"] != '' and $info["userfree"] != '0' ): echo ($info["userfree"]); else: ?>-<?php endif; ?>
                   ...
                   <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tblist">
                    ...
                    <tbody>
                     <?php if(is_array($order_detail)): $i = 0; $__LIST__ = $order_detail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="">
                            ...
                            <td class="ltd">
                                <?php if($vo['guige'] != '' or $vo['guige'] != 0 ): echo ($vo["guige"]); ?>
                                    <?php else: ?>
                                    暂无规格<?php endif; ?>
						...
						<!--	<td>
                            ...
                            <td>
                                <?php if($info['freetype'] == 0): ?>自提<?php if($info['supportmetho'] == 3): ?>(货到付款)<?php endif; ?>
                                    <?php elseif($info['freetype'] == 1): ?>
                                    配送<?php if($info['supportmetho'] == 3): ?>(货到付款)<?php endif; ?>
                                    <?php else: ?>
                                    配送<?php if($info['supportmetho'] == 3): ?>(货到付款)<?php endif; endif; ?>
      ...
      <!--  <a data-height="130" data-width="400" data-id="add" data-title="添加收货地址" data-uri="/index.php?g=admin&amp;m=address&amp;a=add" href="javascript:void(0);" class="add fb J_showdialog"><em>添加收货地址</em></a>-->
		...
		<div class="mt10" style="text-align:center;">
		  <?php switch($info["status"]): case "1": ?><!-- 待付款-->
               ...
               <!--<a data-height="130" data-width="650"  data-id="add" data-title="配送管理" data-uri="<?php echo U('item_order/fahuo',array('id'=>$info['id']));?>" href="javascript:void(0);" class="add fb J_showdialog">-->
                   <input type="button" value="配送" id="dosubmit" name="dosubmit" class="btn btn_submit" >
              <!-- </a>--><?php break;?>  
                <?php case "3": ?><a href="<?php echo U('item_order/status',array('orderId'=>$info['orderId'],'status'=>4));?>"><input type="button" value="设为已收货" id="dosubmit" name="dosubmit" class="btn btn_submit" ></a><?php break;?>
...
<script>
    $('#dosubmit').click(function(){
        ...
        $.post("<?php echo U('item_order/peisong');?>",{id:id},function(data){
            if(data == 1){
                alert('配送成功！');
                window.location.replace("<?php echo U('item_order/index');?>");
            } else {
                alert('配送失败！');
            }
        },'html')
