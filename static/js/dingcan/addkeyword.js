		...
		$.post(url,null,function(data){
			$.each(data,function(i,v){
				...
				zsc_html = "<div class=\"edit-info\">"+
						   ...
						   "<input type=\"hidden\" value=\"\" class=\"addconts\" id=\"zsc_conts0\" />"+
						   ...
						   "</div>"+
						   "<div class=\"replycon\">"+
						   "<textarea id=\"zsc_conts\" onkeyup=\"zsc_contsinfo()\" onchange=\"zsc_contsinfo()\" style=\"width:395px;height:137px; border:#ccc solid 1px;\"></textarea>"+
						   "</div>"+
						   "<div style=\" padding-top:45px; clear:both;\">"+
				...
				if(v.type==1){
					...
					$.each(v.titles,function(i,info){
						if(i>0){
							...
							//$('#zsc_conts').val(v.continfo[i]);
						}
						$('#zsc_t'+i).val(info);
						$('#zsc_i'+i).val(v.imageinfo[i]);
						$('#zsc_urls'+i).val(v.linkinfo[i]);
						//$('#zsc_conts'+i).val(v.continfo[i]);
					});
				}
			});
		},'json');
...
//		var editor;
//		editor = KindEditor.create('.editortext');
	}
}
...
function zsc_addimg(num){
	if($('.simgcon').length<9){
		...
		var zsc_html = "<a href=\"javascript:;\" class=\"simgcon\" id=\"zsc_imgs"+num+"\">"+
					   ...
					   "<input type=\"hidden\" value=\"\" id=\"zsc_conts"+num+"\" class=\"addconts\"/>"+
...
function zsc_editimg(num){
	...
	if(0==zsc_count){
		...
		$('#zsc_conts').val($('#zsc_conts'+zsc_count).val());
...
//内容改变
function zsc_contsinfo(){
	var zsc_conts = $('#zsc_conts').val();
	if(0==zsc_count){
		$('#zsc_conts0').val(zsc_conts);
	}else{
		$('#zsc_conts'+zsc_count).val(zsc_conts);
	}
}
...
function btnsave(num,count){
	...
	if(ktype==1){
		...
		if(count==0){
			...
			$.post(url,{kename:kname,keword:kword,ketype:ktype,titles:arrTitles,imageinfo:arrImgurls,linkinfo:arrCurl,contsinfo:arrconts},function(data){
				alert('添加成功！');
				window.location.reload();
			});
		}else{
			var url = "index.php?g=admin&m=keyword&a=addkeyword&op=update";
			var kid = $('#zsc_iid').val();
			$.post(url,{kename:kname,keword:kword,ketype:ktype,titles:arrTitles,imageinfo:arrImgurls,linkinfo:arrCurl,contsinfo:arrconts,kid:kid},
			function(data){
				alert('修改成功！');
				window.location.reload();
			});
