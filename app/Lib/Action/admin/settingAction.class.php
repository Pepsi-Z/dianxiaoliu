...
class settingAction extends backendAction {
    ...
    public function level_rules(){
        if (IS_POST) {
            $level_rules = $this->_post('level_rules', ',');
            D('setting')->where(array('name'=>'level_rules'))->save(array('data'=>serialize($level_rules)));
    ...
    public function upgrade(){
        if (IS_POST) {
            $level_rules = $this->_post('level_rules', ',');
            $upgrade = $this->_post('upgrade','');

            D('setting')->where(array('name'=>'level_rules'))->save(array('data'=>serialize($level_rules)));
            D('setting')->where(array('name'=>'upgrade'))->save(array('data'=>serialize($upgrade)));
            $this->success(L('operation_success'));
        } else {
            $this->display();
        }
    }
  public function gupgrade(){
        if (IS_POST) {
           
            $gupgrade = $this->_post('gupgrade','');
            D('setting')->where(array('name'=>'gupgrade'))->save(array('data'=>serialize($gupgrade)));
            $this->success(L('operation_success'));
        } else {
            $this->display();
        }
