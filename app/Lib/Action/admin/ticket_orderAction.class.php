...
class ticket_orderAction extends backendAction
{
    public function _initialize()
    {
        parent::_initialize();
        $this->_mod = D('ticket_order');
   ...
   /* public function _before_edit(){
    ...
    public function order_warning() {
        if(true){
            $count = M('ticket_order')->where("warning = 1 and status = 1")->count();
            if($count > 0){
                $arr['state'] = 1;
            }
            $arr['count'] = $count;
    ...
    public function delivery(){
    	...
    	$data['status']=2;
        $data['warning']=0;
    	$a = M('ticket_order')->where('id = '.$id)->save($data);
    	if( $a ){
  ...
  /*  public function _before_update($data){
    ...
    public function updateRemark(){
        ...
        if(M('ticket_order')->where('id='.$id)->save($data)!==false)
        {
            $this->success('修改成功！');
        }else
        {
