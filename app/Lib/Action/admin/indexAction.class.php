...
class indexAction extends backendAction {
    ...
    public function panel() {
        ...
        $tx_money = M('member_money')->where('state=0')->count();
        
        $fukuan= $this->item_order->where('status=1')->count();
        $fahuo= $this->item_order->where(array('delete'=>0,'status'=>2,'type'=>1))->count();
        $yfahuo= $this->item_order->where('status=3')->count();

        $ticket= M('return_integral')->where('status=1')->count();
        $express= M('cityexpress_order')->where('status=1')->count();

        $score= M('score_order')->where('status=1')->count();
        $this->assign('count',
        array('fukuan'=>$fukuan,
        'fahuo'=>$fahuo,
        'yfahuo'=>$yfahuo,
    ...
    public function left() {
        ...
        if ($menuid) {
            ...
            $left_menu[1] = array('id'=>0,'name'=>'交易管理');
            $left_menu[1]['sub'] = array();
            if ($r = $this->_mod->where(array('often'=>9))->select()) {
                $left_menu[1]['sub'] = $r;
            }
        }
