...
class return_integralAction extends backendAction
{
    public function _initialize()
    {
        parent::_initialize();
        $this->_mod = D('return_integral');
    ...
    protected function _search() {
        ...
        ($uname = $this->_request('username', 'trim')) && $map['username'] = array('like', '%'.$uname.'%');
        ($sname = $this->_request('sname', 'trim')) && $map['sname'] = array('like', '%'.$sname.'%');
   ...
   /* public function _before_edit(){
    ...
    public function order_warning() {
        if(true){
            $count = M('return_integral')->where("warning = 1 and status = 1")->count();
            if($count > 0){
                $arr['state'] = 1;
            }
            $arr['count'] = $count;
    ...
    public function delivery(){
    	...
    	$data['status']=2;
        $data['warning']=0;
    	$a = M('return_integral')->where('id = '.$id)->save($data);
    	if( $a ){
  ...
  /*  public function _before_update($data){
    ...
    public function updateRemark(){
        ...
        if(M('return_integral')->where('id='.$id)->save($data)!==false)
        {
            $this->success('修改成功！');
        }else
        {
